...
def insert_user(conn, user_data):
    sql_insert_user = """INSERT INTO users (name, mobile_number, email, company_name, designation, password)
                        VALUES (?, ?, ?, ?, ?, ?);"""
    try:
        cur = conn.cursor()
...
def authenticate_user(conn, email, password):
    ...
    cur = conn.cursor()
    cur.execute(sql_authenticate_user, (email, password))
    user = cur.fetchone()
    return user


def signup(name, mobile_number, email, company_name, designation, password):
    ...
    if conn is not None:
        ...
        result = insert_user(conn, user_data)
        conn.close()
        return result
    else:
...
def LoggedIn_Clicked(email, password):
    if login(email, password):
        ...
        st.toast("Login successful. Redirecting to main app...")
    else:
...
def show_signup_page():
    ...
    if st.button("Sign Up"):
        ...
        result = signup(
            name, mobile_number, email, company_name, designation, password
        )
        if result == True:
...
#     #             elif not validate_password(password):
#     #                 st.error("Password must be at least 8 characters long.")
#     #             result = signup(name, mobile_number, email, company_name, designation, password)
#     #             if result == True:
#     #                 st.success("Sign Up completed")
#     #             else:
#     #                 st.error(f"Error: {result}")
...
#     #                 st.session_state.logged_in = True
#     #                 st.success("Login successful. Redirecting to main app...")
#     #                 # Streamlit does not support opening other scripts directly
#     #                 # You can use the streamlit.components.v1.html function to create custom redirects
#     #                 st.rerun()
#     #                 st.empty()
#     #                 return
#     #             else:
